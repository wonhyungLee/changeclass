...
| 성별 | `성별` | `남/남성/남자/M`, `여/여성/여자/F` 등을 인식합니다(그 외는 기타로 처리). |
...
### Q1. 업로드 후 “파일을 읽는 중 오류가 발생했습니다” 라고 뜹니다.

- 파일 형식이 `.xlsx/.xls/.csv`인지 확인하세요.
- 첫 번째 시트(첫 탭)에 데이터가 있고, 첫 행에 컬럼명(헤더)이 있는지 확인하세요.
- 엑셀에서 병합 셀/특수 서식이 많으면 파싱이 실패할 수 있으니, 가능하면 단순 표 형태로 정리해 다시 시도하세요.
...
- 성별 값은 `남/남성/남자/M`, `여/여성/여자/F` 등을 인식합니다.
- 그 외의 표기(예: 빈 값, 특수 표기)는 “기타/미상”으로 처리될 수 있습니다.
...
- 엑셀 처리는 사이트 내부 모듈로 처리되며, 별도의 외부 CDN 의존 없이 동작합니다.
